...
cap = cv2.VideoCapture(0)
print("Opening camera. Press 's' 3 times (Front, Left, Right) to capture different angles.")

embedding_list = []

while len(embedding_list) < 3:
    ret, frame = cap.read()
    if not ret:
        print("Camera error.")
        break
    ...
    if key == ord('s'):
        ...
        if embedding is not None:
            ...
            print(f"Captured angle {len(embedding_list)} of 3.")
        else:
            print("No face detected. Try again.")
    elif key == ord('q'):
        print("Quitting...")
...
# Check against all registered embeddings
found = False
for record in collection.find():
    encrypted_data = record["embedding"]
    key = record["key"]
    ...
    for new_embedding in embedding_list:
...
if not found:
    ...
    key = Fernet.generate_key()
    encrypted_vector = encrypt_face_vector(embedding_list[0], key)  # Store the first (frontal) as main
    ...
    collection.insert_one({
        ...
        "embedding": encrypted_vector,
        "key": key,
        "embedding_list": [pickle.dumps(vec) for vec in embedding_list]  # Optional: store all
    })

    print(f"{name} registered successfully with multiple angles.")
else:
    print(f"Face already Registered!!")
